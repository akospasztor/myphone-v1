...
unsigned char about[] = "   myPhone v1.0 ~~~ written by Akos Pasztor   ";

/*** VARIABLES ****************************************************************/
unsigned char disp[LCD_MAX + 1];     //lcd buffer (ddram mirror)
unsigned int ptr_disp;               //lcd buffer pointer

int allapot;                         //device state     0:standby     1:keypad    2:contacts    3:call
int gsm_allapot;                     //gsm state
unsigned char service[17];           //gsm service name
...
char * pch;                          //GSM rec buffer search pointer
...
{       
    ...
    //port directions
    PORTC |= b00000110;     //GSM_FET: OFF, amplifier OFF
...
void disp_wchar(unsigned char c)
{
    ...
    switch(ptr_disp)
    {
        case 0x0F: //1. sor vege
        ptr_disp = 0x28;
        break;
        
        case 0x37: //2. sor vege
        ptr_disp = 0x10;
        break;
        
        case 0x1F: //3. sor vege
        ptr_disp = 0x38;
        break;
...
void usart_init(void)
{
    ...
    // FTDI & GSM Baud rate: 2400 - UBRR: 319 (0x13F) @ 12.288 MHz
...
unsigned int SPI_count_rows(void)
{
    ...
    do
    {
        ...
        if(i >= 64 && error) { return 0; }              // if first 64 char not contain 0x00 then its surely empty
        ++i;
    } while(c != ';');
...
{     
    ...
    {   
        ...
        if(FLAG_key)
        {
            //ANSWER button is active during incoming call
            ...
            //END button is active during call
            if(allapot == 3 && FLAG_key == 14)
            {
                //PWM
                TCCR0 = 0x00;
            ...
            {                
                ...
                if(FLAG_key == 11) { addkey('*'); allapot = 1; }
                if(FLAG_key == 12) { addkey('#'); allapot = 1; }
                
                //CTRL KEYS
                if(FLAG_key == 13)
                {
                    if(FLAG_gsm_idle)      //---DIAL-----------------------
                    {
                        move_row(3,4);
                        write_row(1,"                ");
                        write_row(2,"                ");
